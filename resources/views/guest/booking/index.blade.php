...
@section('title', 'Data Booking Tamu')
...
<div class="col-md-9">
    <div class="panel panel-default">
        <div class="panel-heading">
            @yield('title')
            <div class="pull-right"><a href="{{ route('guest-booking.create') }}" class="btn btn-info" style="margin-top: -7px;" role="button">Tambah Booking Baru</a></div>
        </div>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr class="{{ ($item->cancellation) ? 'danger' : ($item->status ? 'info' : '') }}" title="{{ ($item->cancellation) ? 'Dibatalkan' : '' }}">
                        ...
                        <td>
                            <a href="{{ route('guest-booking.show', $item->id) }}">{{ date('d-m-Y', strtotime($item->date)) }}</a>
                        </td>
                        ...
                        <td>
                            @if ((!$item->status AND !$item->cancellation) AND !$item->cancellation)
                                <a href="{{ route('guest-booking.destroy', $item->id) }}" title="Batalkan"
                                   onclick="event.preventDefault(); document.getElementById('cancel-form-{{$item->id}}').submit();"
                                   style="color: red; margin-left: 15px;">
                                    <i class="fa fa-times" aria-hidden="true"></i>
                                </a>
                                <form id="cancel-form-{{$item->id}}" action="{{ route('guest-booking.destroy', $item->id ) }}" method="post" style="display: none;">
                                    {{ method_field('PUT') }}
                                    {{ csrf_field() }}
